...
func run_tests() -> void:
	...
	game.get_node("MichelinManager").buy_upgrade("vip_spawn_1")
	game.get_node("MichelinManager").buy_upgrade("vip_group_2")
	...
	check(game.get_node("Restaurant").waiter_manager.get_hired_count() == 0, "New game must remove hired waiters")
